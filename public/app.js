...
async function loadBookToc(bookId) {
  ...
  try {
    const res = await fetch(`/api/book/${bookId}/toc`);
...
window.loadChapter = async function(href, chapterIndex, targetParagraphId = null) {
  ...
  try {
    const res = await fetch(`/api/book/${state.activeBookId}/chapter?href=${encodeURIComponent(href)}`);
...
async function handleInBookSearch(query) {
  ...
  try {
    const res = await fetch(`/api/book/${state.activeBookId}/search?q=${encodeURIComponent(query)}&limit=30`);
